...
exports.insert = async (type, info) => {
    ...
    if (type == 'conversation') {
        if (verify) return
        simih.push({ msg: info.message.conversation.toLowerCase(), command: [] })
        fs.writeFileSync('./database/json/simi.json', JSON.stringify(simih, null, 4))
    } else if (type == 'extendedTextMessage') {
        if (verify) {
            verify.command.push(info.message.extendedTextMessage.text.toLowerCase())
            fs.writeFileSync('./database/json/simi.json', JSON.stringify(simih, null, 4))
        } else {
            simih.push({ msg: keyword.toLowerCase(), command: [info.message.extendedTextMessage.text.toLowerCase()] })
            fs.writeFileSync('./database/json/simi.json', JSON.stringify(simih, null, 4))
        }
    }
}
...
exports.response = async (key) => {
    ...
    return position ? simih[position].command[Math.floor(Math.random() * simih[position].command.length)] : false
}