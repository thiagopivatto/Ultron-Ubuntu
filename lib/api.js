...
const { insert, response } = require('../lib/simi.js');
const samih = JSON.parse(fs.readFileSync('../database/db/simi.json'));
const samih2 = JSON.parse(fs.readFileSync('../lib/simi.json'));

module.exports = {
insert, response, simih, samih, samih2, 

    textoParaVoz: async (idioma, texto)=>{
        return new Promise((resolve, reject) => {
            //const ttsPtBr = require('node-gtts')('pt-br')
            const ttsPt = require('node-gtts')('pt')
            const ttsEnUs = require('node-gtts')('en')
            //const ttsEnUk = require('node-gtts')('en-uk')
            ...
            //if (idioma == 'pt-br') {
            //    ttsPtBr.save(path.resolve('media/tts/resPtBr.mp3'), texto, function () {
            //        resolve(path.resolve('media/tts/resPtBr.mp3'))
            //    })
            if (idioma == 'pt') {
                ...
                ttsEnUs.save(path.resolve('media/tts/resEnUs.mp3'), texto, function () {
                    resolve(path.resolve('media/tts/resEnUs.mp3'))
                })
            //} else if (idioma == 'en-uk') {
            //    ttsEnUk.save(path.resolve('media/tts/resEnUk.mp3'), texto, function () {
            //        resolve(path.resolve('media/tts/resEnUk.mp3'))
            //    })
            } else if (idioma == 'jp') {
                ttsJp.save(path.resolve('media/tts/resJa.mp3'), texto, function () {
                    resolve(path.resolve('media/tts/resJa.mp3'))
                })
            } else if (idioma == 'it') {
                ...
                ttsFi.save(path.resolve('media/tts/resFi.mp3'), texto, function () {
                    resolve(path.resolve('media/tts/resFi.mp3'))
                })
            } else if (idioma == 'fr') {
                ttsFr.save(path.resolve('media/tts/resFr.mp3'), texto, function () {
                    resolve(path.resolve('media/tts/resFr.mp3'))
                })
            } else if (idioma == 'de') {
                ttsDe.save(path.resolve('media/tts/resDe.mp3'), texto, function () {
                    resolve(path.resolve('media/tts/resDe.mp3'))
                })
            } else if (idioma == 'zh') {
                ttsZh.save(path.resolve('media/tts/resZhCn.mp3'), texto, function () {
                    resolve(path.resolve('media/tts/resZhCn.mp3'))
                })
            } else if (idioma == 'ar') {
                ttsAr.save(path.resolve('media/tts/resAr.mp3'), texto, function () {
                    resolve(path.resolve('media/tts/resAr.mp3'))
                })
            }
            else {
    ...
    obterNoticiasUS: async () => {
        try {
            let res = await axios.get(`http://newsapi.org/v2/top-headlines?country=us&apiKey=${process.env.API_NEWS_ORG.trim()}`), resposta = []
            for (var noticia of res.data.articles) {
                resposta.push({
                    titulo: noticia.title,
                    descricao: noticia.description,
    ...
    obterTraducao : async (texto, idioma)=>{
        try {
            var idiomasSuportados = ["pt", "es", "en", "ja", "it", "ru", "ko", "fi", "fr", "de"]
    ...
    //    try {
    //        const result = await axios.get(`https://api.hgbrasil.com/weather?key=89ebc412&city_name=${local}`)
    //        const respostaClima = {
    //            cidade: result.data.results.city_name,
    //            temperatura: result.data.results.temp,
    //            dia: result.data.results.date,
    ...
    // obterVerdadeOuDesafio : async(vddousf)=>{
    //     try {
    //         const vod = await axios.get("https://gist.github.com/deepakshrma/9498a19a3ed460fc662c536d138c29b1")
    //         const nivel = vod.data.level
    //         const indexVOD = vod.data.id
    //         const tipo = vod.data.type
    //         const verdade = vod.data.summary
    //         var idAleatorio = Math.floor(Math.random() * vod.indexTruth.length)
    //         var resposta = criarTexto(msgs_texto.diversao.vod.resposta, vod[indexVOD].level, vod[indexVOD].summary)
    //         return resposta
            
    //         vod.summary, verdadeAleatoria = Math.floor(Math.random() * cartas.cartas_pretas.length), cartaPretaEscolhida = cartas.cartas_pretas[cartaPretaAleatoria], cont_params = 1
    //         if(cartaPretaEscolhida.indexOf("{p3}" != -1)){cont_params = 3}
    //         else if(cartaPretaEscolhida.indexOf("{p2}" != -1)) {cont_params = 2}
    //         else {cont_params = 1}
    //         for(i = 1; i <= cont_params; i++){
    ...
    //         return resposta
    //     } catch(err){
    //         consoleErro(err.message, "API obterVerdadeOuDesafio")
    //         throw new Error(msgs_texto.diversao.vod.erro_servidor)
    //     }
    // },
    ...
    obterInfoDDI: async(DDI) => {
        try {
            const githubGistDDI = await axios.get("https://github.com/cristianoascari/paises-no-formato-JSON/blob/master/countriesJson_ptBR.json")
            const paises = githubGistDDI.data
            const indexDDI = paises.findIndex(data => data.DDI.includes(DDI))
            if (indexDDI != -1) {
                var resposta = criarTexto(msgs_texto.utilidades.ddi.resposta, paises[indexDDI].nome)
                return resposta
            } else {
                throw new Error(msgs_texto.utilidades.ddi.nao_encontrado)
            }
        } catch (err) {
            var errors = [msgs_texto.utilidades.ddi.nao_encontrado]
            if (!errors.includes(err.message)) {
                consoleErro(err.message, "API obterInfoDDI")
                throw new Error(msgs_texto.utilidades.ddi.erro_servidor)
            } else {
                throw err
            }
        }
    ...
    obterMembroInfo: async () => {
        try {
            const dadosMembro = await axios.get("https://api.hgbrasil.com/geoip?fields=only_results,address,country_name,region,city&key=89ebc412&address=remote")
            return dadosMembro.data
        } catch (err) {
            consoleErro(err.message, "API dados Membro")
            throw new Error(msgs_texto.admin.membroinfo.erro_membroinfo)
        }
    },

    obterAcao: async acao => {
        try {
            const acaoURL = await axios.get("https://api.hgbrasil.com/finance/stock_price?key=89ebc412&symbol=${local}")
            var respostasAcoes = {
                Ticker: symbol,
                NomeEmpresa: name,
                CNPJ: document,
                ...
                Preço: price,
                MoedaReferência: currency,
                Variação: change_percent,
                ÚltimaAtualização: updated_at
            }
            return respostasAcoes
        } catch (err) {
            consoleErro(err.message, "API obterAcoes")
            throw new Error(msgs_texto.utilidades.acao.erro_resultado)
        }
    },
