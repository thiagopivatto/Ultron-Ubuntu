...
const { insert, response } = require('./simi');
...
// insert, response, simih, samih, samih2, 
    ...
    // textoParaVozIBM: async (idioma, texto)=>{
    //     return new Promise((resolve, reject) => {
    ...
    //         text_to_speech.synthesize(params).pipe(fs.createWriteStream('output.wav')).on('close',
    //         function() {
    //         var saida = sh.exec('play output.mp3', {silent:true}).output;
    //         });
    //     }},
    ...
    obterResultadoAI : async pesquisaAI =>{
        try{
            ...
            var errors = [msgs_texto.diversao.falae.erro_servidor]
            if(!errors.includes(err.message)){
                consoleErro(err.message, "API obterResultadoAI")
                throw new Error(msgs_texto.diversao.falae.erro_servidor)
            } else {
                throw err
            }
        }
